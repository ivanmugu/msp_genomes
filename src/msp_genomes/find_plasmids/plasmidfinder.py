"""Find plasmids incompatibility types

This script was duplicated from:
    ...
    for dbs_info in json_data.values():
        for db_name, db_info in dbs_info.items():
            ...
            for gene_id, gene_info in sorted(
                db_info.items(), key=lambda x: (x[1]["plasmid"], x[1]["accession"])
            ):
...
##########################################################################
def argparser_plasmidfinder():  # Modified by IMG
    parser = ArgumentParser()
    parser.add_argument(
        "-i", "--infile", help="FASTA or FASTQ input files.", nargs="+", required=True
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--tmp_dir",
        help=(
            "Temporary directory for storage of the results "
    ...
    parser.add_argument(
        ...
        "--extented_output",
        help=(
            "Give extented output with allignment files, "
...
# MAIN
##########################################################################
def plasmidfinder_runner(args):  # Modified by IMG
    # = Arguments modified by IMG ====================================== #
    args.db_path = PLASMIDFINDER_DB  # path to plasmidfinder_db
    args.tmp_dir = TMP_DIR  # Provide path to tmp_dir
    args.extented_output = True  # Extended (extented) output True
    # ================================================================== #
    ...
    if file_format == "fastq":
        ...
        if find_executable(method_path) is None:
            sys.exit(
                "No valid path to a kma program was provided. Use the -mp "
        ...
        if find_executable(method_path) is None:
            sys.exit(
                "No valid path to a blastn program was provided. Use the "
    ...
    if args.extented_output:
        ...
        for species, dbs_info in json_results.items():
            for db_name, db_hits in dbs_info.items():
                ...
                for gene_id, gene_info in sorted(
                    db_hits.items(), key=lambda x: (x[1]["plasmid"], x[1]["accession"])
                ):
...
if __name__ == "__main__":  # Modified by IMG
    # Parse command line input
    args = argparser_plasmidfinder()
    print(args)
    # # Run plasmid finder
    # plasmidfinder_runner(args)
    # # Clear tmp folder
