...
from msp_genomes.utils.config import MLST_DB, TMP_DIR
...
def get_read_filename(infiles):
    ...
    if sample_name == "":
        sys.error(
            "Input error: sample names of input files, {} and {}, \
                   ...
                   or input them seperately.".format(
                infiles[0], infiles[1]
            )
        )
...
def st_typing(st_profiles, allele_matches, loci_list):
    ...
    for locus in allele_matches:
        ...
        if (
            ...
            st_marks.append("!")
            for allele_name, hit_info in allele_matches[locus][
                "alternative_hit"
            ].items():
...
def make_aln(
    species, file_handle, allele_matches, query_aligns, homol_aligns, sbjct_aligns
):
...
# = MODIFIED BY IMG =====================================================================
# All the code below was modified by IMG
# =======================================================================================
...
def mlst_argparser():
    ...
    parser.add_argument(
        ...
        help="Give extented output with allignment files, template and query hits in fasta and\
                            a tab seperated file with allele profile results",
        action="store_true",
    )
    ...
    parser.add_argument(
        "-st",
        "--save_tmp",
        help="if set the tmp folder will be saved - " "deleted by default",
        action="store_true",
        default=False,
    )
    parser.add_argument(
...
def mlst_runner(args: Namespace) -> None:
    if args.quiet:
        f = open("/dev/null", "w")
        sys.stdout = f
    ...
    if extented_output:
        ...
        for locus, allele_info in allele_matches.items():
            ...
            header = ">{}:{} ID:{}% COV:{}% Best_match:{}\n".format(
                locus,
                match,
                allele_info["identity"],
                allele_info["coverage"],
            ...
            if "alternative_hit" in allele_info:
                for allele_name in allele_info["alternative_hit"]:
                    header = ">{}:{} ID:{}% COV:{}% Best_match:{}\n".format(
                        locus, "PERFECT MATCH", 100, 100, allele_name
                    )
